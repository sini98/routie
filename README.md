...
홈 화면은 **오늘 외출 / 지정 외출 / 최근 작성한 일정** 3가지로 단순하게 유지하고, 지역/위치를 정하는 방법은 두 화면의 성격에 맞게 나눠뒀습니다: 오늘 외출은 "현재 위치 기반으로 빠르게 시작", 지정 외출은 "원하는 지역을 직접 검색해서 선택". 이 둘은 **서로 다른 상태를 씁니다** — 아래 "두 값을 분리한 이유" 참고.
...
- **오늘 외출 — 현재 위치로 빠르게 시작** (`useAutoLocate.ts`, `OutingScreen.tsx`, `NaverMap.tsx`의 `currentLocationOnly` prop): 오늘 날짜 일정에 장소가 **하나도 없을 때만** 동작합니다(장소가 있으면 절대 건드리지 않습니다).
  ...
  - **위치 조회 중에는 지도가 안 보입니다.** `useAutoLocate`의 `status`가 `"checking"`인 동안 `OutingScreen.tsx`가 지도 위에 "현재 위치를 불러오는 중..." 불투명 오버레이를 덮습니다. 지도(및 네이버 지도 SDK 스크립트) 자체는 오버레이 뒤에서 평소처럼 계속 로드/초기화되므로, 위치 조회와 스크립트 로딩이 동시에 진행돼 체감 속도는 그대로입니다 — 다만 최종 결과(성공 시 현재 위치, 실패 시 서울)가 정해지기 전까지는 사용자에게 아무것도 보여주지 않을 뿐입니다.
...
- **지정 외출 — 원하는 지역 직접 검색** (`RegionSearch.tsx`): 달력에서 아직 일정이 없는 날짜를 고르면, 요약 카드에 지역 검색창(`지역명(시·군·구·읍·면·동)을 검색하세요.`)이 나타납니다. "대구", "강남구", "불광동"처럼 검색해서 고르면 그 좌표가 기본 지역(`routie:defaultRegion`, localStorage에 저장됨)으로 저장되고 곧바로 그 날짜의 외출 화면(`/outing/{날짜}`)으로 이동합니다. 이미 일정이 있는 날짜에는 표시되지 않습니다(기존 일정/위치를 덮어쓰지 않기 위해서입니다).
  - 예전에는 홈 화면에 이 검색창이 있었고, 그보다 더 예전에는 위치 권한을 거부하면 고정된 8개 도시(서울/대구/부산/거제/제주/광주/대전/인천) 중에서만 골라야 했습니다(목록에 없는 지역은 선택 불가). 지금은 오늘/지정 외출의 역할이 명확히 나뉘면서, 지역 검색은 지정 외출과 장소 추가 화면(`LocationPicker`)에만 있습니다.
- **`currentLocation`(오늘 외출)과 `routie:defaultRegion`(지정 외출/최초 실행)을 완전히 분리한 이유**: 예전에는 `useAutoLocate`가 현재 위치를 얻으면 `routie:defaultRegion`에 그대로 저장했습니다. 그런데 이 값은 지정 외출 지역 검색과 **같은 localStorage key를 공유**하고 있어서, 예를 들어 지정 외출 달력에서 (테스트든 실제든) "대구"를 한 번 검색해두면, 그 값이 계속 남아있다가 며칠 뒤 "오늘 외출"에 들어갔을 때도 "저장된 기본 지역"으로 다시 읽혀서 지도가 대구로 뜨는 문제가 있었습니다(실제 위치는 서울이고 권한도 허용돼 있었는데도). `useAutoLocate`가 반환하는 `currentLocation`은 이제 이 훅 안의 컴포넌트 state일 뿐 `routie:defaultRegion`을 읽지도 쓰지도 않고(`NaverMap`에도 `currentLocationOnly`라는 별도 prop으로 전달), `routie:defaultRegion`은 지정 외출/최초 실행(`RegionSetup`) 전용으로만 남습니다. 그래서 `routie:defaultRegion`을 앱 시작 시 강제로 지우는 마이그레이션은 넣지 않았습니다 — 그 값은 여전히 지정 외출에서 정상적으로 쓰이는 값이라, 지우면 오히려 "지정 외출에서 검색한 지역을 기억하는" 기능이 깨집니다. 이미 들어있는 예전 값(예: 테스트로 검색했던 대구)이 있어도, 이제 오늘 외출은 그 값을 아예 보지 않으므로 더 이상 영향을 주지 않습니다.
...
src/
  ...
  components/
    ...
    RegionSearch.tsx      # 지역 검색 입력창 (검색 → 기본 지역 저장 → targetPath로 이동). 지정 외출(달력)에서 사용
  ...
  lib/
    ...
    geocode.ts (geocodeQuery — /api/geocode 지역명/주소→좌표, searchPlaces — /api/search-place 장소명→이름/주소만, reverseGeocodeQuery — /api/reverse-geocode 좌표→장소명 제안; LocationPicker는 셋 다, RegionSearch는 geocodeQuery만 사용)
public/
